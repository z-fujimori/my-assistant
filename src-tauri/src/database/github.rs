  ...
  let mut last_date: String;
    ...
    if let (Some(owner), Some(repo)) = (parts.get(3), parts.get(4)) {
      ...
      let branches = get_activ_branch(owner.to_string(), repo.to_string(), pool, project.id).await?;
			...
			let mut total_code_chanses = CommitCodeDiff{total: 0, additions: 0, deletions: 0};
			for b in branches {
				let (commit_stats, branch_last_date) = get_commit_info(owner.to_string(), repo.to_string(), b.name.clone(), prev_date.clone()).await?;
...
pub(crate) async fn get_commit_info(owner:String, repo:String, branch: String, prev_date:String) -> Result<(CommitCodeDiff, String)> {
	let client = Client::new();
	...
	for c in commit_datas {
		...
		let las_date: DateTime<Utc> = last_date.parse().expect("Invalid date format");
		if change_date > las_date {
			last_date = commit_stats.commit.committer.date;
		}
	}
	Ok((return_stats, last_date))
}
